...
function avatax_lookup_tax($order, $products) {
  ...
  $state_code = tep_get_zone_code($country_code, $region, $state) ;
  ...
  $request_body = array(
    'Client' => 'osCommerce-AvalaraInc,2.34',
    'CompanyCode' => MODULE_ORDER_TOTAL_AVATAX_CODE,
    'DetailLevel' => 'Tax',
    'Commit' => $commit_tep,
    'DocType' => $doc_type,
    'DocCode' => ('tep-' . $new_order_id . ''),
...
$prod_total = 0;
  ...
  $response = tep_tax_avatax_post('/tax/get', $request_body);
  ...
  if (is_array($response) && $response['body']) {
    ...
    if ($result['ResultCode'] == 'Success') {
      ...
      foreach ($result['Messages'] as $msg) {
	    $messageStack->add('checkout_confirmation', 'AvaTax message: ' . $msg['Severity'] . ' - ' . $msg['Source'] . ' - ' . $msg['Summary'] . '', 'error');
    ...
    $messageStack->add('checkout_confirmation', 'AvaTax error: AvaTax did not get a response.', 'error');
    if ($messageStack->size('checkout_confirmation') > 0) echo $messageStack->output('checkout_confirmation');
    return FALSE;
  }
}
 ...
 */
function tep_tax_avatax_get($endpoint, $parameters, $base_url = '', $account = '', $license = '') {
  $querystring = '';
  ...
  list($mode, $version, $account, $license, $base_url) = tep_tax_avatax_get_config($account, $license, $base_url);
  ...
  $body = curl_exec($curl);
  if ($body === FALSE) {
	$messageStack->add('checkout_confirmation', 'AvaTax request failed. This may be an out of date SSL certificates on your server - ' . curl_error($curl) . '', 'error');
  }
 ...
 *   Returns an associative array containing 'meta' and 'body' elements.
 */
function tep_tax_avatax_post($endpoint, $data, $base_url = '', $account = '', $license = '') {
  $curl_opts = array(
    CURLOPT_RETURNTRANSFER => TRUE,
    CURLOPT_FOLLOWLOCATION => FALSE,
    CURLOPT_MAXREDIRS => 10,
 ...
 */
function tep_tax_avatax_get_config($account = '', $license = '', $base_url = '') {
