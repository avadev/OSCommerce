 ...
 * @version 2.3.R 2014-01-27 16:00:00Z ajeh $
 */
...
function tep_avatax_commit_tax($order, $products, $oID) {
 ...
 */
function tep_avatax_commit_transaction($oID) {
  _tep_avatax_update($oID, 'commit');
}

/**
 * DELETE AvaTax transaction for $oID.
 */
function tep_avatax_cancel_transaction($oID) {
  _tep_avatax_update($oID, 'cancel');
}

/**
 * Send Commit/Cancel operation to AvaTax.
 */
function _tep_avatax_update($oID, $type = 'commit') {
  ...
  switch ($type) {
    case 'commit':
      ...
      $order = new order($oID);
	  tep_avatax_commit_tax($order, $order->products, $oID);
      break;
    ...
    case 'cancel':
      ...
      $response = tep_tax_avatax_post('/tax/cancel', $body);
